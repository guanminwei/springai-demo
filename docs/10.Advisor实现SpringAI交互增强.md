...
根据项目文档和 Spring AI 官方文档，以下是 Spring AI 内置的常用 Advisor 及其使用场景：
...
| Advisor | 使用场景 |
|---------|---------|
| **`MessageChatMemoryAdvisor`** | 将对话历史作为 `Message` 列表直接注入到提示中，保留完整的对话结构。适用于需要**多轮对话上下文**的场景，如客服机器人、聊天助手。项目中多处使用（如 [A01-memory-jdbc-mysql](file:///d:/hzzh/project/AI/spring-ai-demo-master/spring-ai-demo-master/advance-projects/A01-memory-jdbc-mysql/src/main/java/com/git/hui/springai/advance/mvc/ChatController.java)、[S09-chat-client](file:///d:/hzzh/project/AI/spring-ai-demo-master/spring-ai-demo-master/S09-chat-client/src/main/java/com/git/hui/springai/mvc/ChatController.java)）。 |
| **`PromptChatMemoryAdvisor`** | 将历史记忆内容**整合到系统提示词（system text）中**，而非作为独立消息。适用于模型不支持多消息输入、或希望压缩上下文长度的场景。 |
| **`VectorStoreChatMemoryAdvisor`** | 从**向量存储库**中检索相关记忆并注入系统提示词。适用于**长期记忆**场景，能从海量历史对话中语义检索相关信息。 |

### 二、RAG（检索增强生成）相关

| Advisor | 使用场景 |
|---------|---------|
| **`QuestionAnswerAdvisor`** | 基于向量存储实现 **RAG 问答**，自动将用户问题做相似度检索，将相关文档片段注入提示词。适用于**知识库问答、文档问答**等场景。项目中 [D05-rag-qa-bot](file:///d:/hzzh/project/AI/spring-ai-demo-master/spring-ai-demo-master/app-projects/D05-rag-qa-bot) 即使用了此模式。 |

### 三、日志/监控相关

| Advisor | 使用场景 |
|---------|---------|
| **`SimpleLoggerAdvisor`** | 打印请求和响应的**日志**，用于开发调试。支持自定义格式化函数。项目中广泛使用，如 [S09-chat-client](file:///d:/hzzh/project/AI/spring-ai-demo-master/spring-ai-demo-master/S09-chat-client/src/main/java/com/git/hui/springai/mvc/ChatController.java#L264-L267) 中通过 lambda 自定义前缀格式。 |

### 四、内容安全相关

| Advisor | 使用场景 |
|---------|---------|
| **`SafeGuardAdvisor`** | 基础**内容安全防护**，拦截并阻止模型生成有害、敏感或不恰当的内容。适用于面向公众的 AI 应用，作为输入/输出的安全过滤层。 |

### 五、工具调用相关

| Advisor | 使用场景 |
|---------|---------|
| **`ToolCallAdvisor`** | 管理**工具调用（Function Calling）**的 Advisor，协调模型与外部工具的交互。适用于需要模型调用外部 API/工具的场景，如 [T02-manual-qa-cli-robot](file:///d:/hzzh/project/AI/spring-ai-demo-master/spring-ai-demo-master/v2/T02-manual-qa-cli-robot/src/main/java/com/git/hui/springai/app/T02Application.java#L8) 中使用。 |
...
ChatClient chatClient = ChatClient.builder(chatModel)
    .defaultAdvisors(
        ...
        QuestionAnswerAdvisor.builder(vectorStore).build()      // RAG 问答
    )
    .build();
```

> **注意**：自定义 Advisor 时需同时考虑**同步（call）** 和**流式（stream）** 两种模式，因为 Advisor 链对两种调用方式都会生效。